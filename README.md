...
| `artifact-binding` | no | HTTP-Artifact binding (SOAP `ArtifactResolve`) + `BackchannelClient`. Pulls in `sha1` for the spec-mandated SourceID (Bindings §3.6.4); independent of `weak-algos`. |
...
- **Weak algorithms are feature-gated.** SHA-1 signature and digest algorithms (RSA-SHA1, DSA-SHA1, `DigestAlgorithm::Sha1`) and RSA-PKCS1-v1.5 key transport are unavailable unless `weak-algos` is enabled. Even when compiled in, the per-peer `PeerCryptoPolicy` allow-list still gates acceptance at validation time. Note that the `sha1` *crate* may be present without `weak-algos`: `artifact-binding` uses it for the spec-mandated artifact `SourceID` (Bindings §3.6.4), which is an identity-matching tag rather than a security primitive. Check the enabled features, not the dependency list — `cargo tree -p saml -f "{f}" --depth 0`.
