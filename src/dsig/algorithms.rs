...
impl PeerCryptoPolicy {
    ...
    /// Policy for verifying signed SAML metadata.
    ///
    /// Identical to [`Self::strong_defaults`] except that every implemented
    /// canonicalization algorithm is accepted. Federation aggregates are
    /// signed once, offline, by tooling this SP does not control, and
    /// inclusive C14N is common in that population — restricting metadata to
    /// exclusive C14N would reject aggregates that verified before per-peer
    /// C14N policy existed. Signature and digest strength are *not* relaxed:
    /// C14N choice does not weaken the signature, whereas the hash does.
    ///
    /// Callers that control their metadata source should pass
    /// [`Self::strong_defaults`] to the `_with_policy` metadata entry points
    /// instead.
