...
pub enum KeyTransportAlgorithm {
    /// RSA-OAEP with the digest selected by `<ds:DigestMethod>` (SHA-1 when
    /// omitted) and the MGF1 hash selected by `<xenc11:MGF>` (MGF1-SHA1 when
    /// omitted). URI:
    /// `http://www.w3.org/2009/xmlenc11#rsa-oaep`.
    RsaOaep,
    /// Legacy RSA-OAEP with the digest selected by `<ds:DigestMethod>` (SHA-1
    /// when omitted) and MGF1-SHA1 fixed by the algorithm URI. URI:
